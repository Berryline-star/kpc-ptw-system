...
interface LoginFormProps {
  /** Where to send the user after a successful sign-in. Already
   * validated as a safe same-origin path by the parent Server Component
   * — see `getSafeCallbackUrl` in `login/page.tsx`. */
  callbackUrl: string;
}
      ...
      <div className="mb-stack-lg text-center">
        ...
        <p className="text-body-sm text-on-surface-variant">
          {callbackUrl !== "/dashboard"
            ? "Sign in to continue to your request."
            : "Access the Digital Permit-to-Work operational safety system."}
        </p>
      </div>

      <form action={formAction} className="space-y-stack-md">
        {/* Carries the originally-requested page through the server
            action so a successful login lands back where the user was
            headed (e.g. /permits/new) instead of always /dashboard. */}
